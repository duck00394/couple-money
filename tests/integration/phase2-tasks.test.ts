...
describe("Phase 2-3～2-8：任務、打卡、獎金（尚未入金）、連續、里程碑、懲罰", () => {
  ...
  const input = (over: Partial<TaskInput> = {}): TaskInput => ({
    title: "英文 30 分鐘", description: "", emoji: "book", scope: "PERSONAL", assigneeId: c.aId, frequency: "DAILY", daysOfWeek: 127,
  ...
  before(async () => {
    ...
    goalId = (await goals.createGoal(c.ctxA, { name: "日本旅行", description: "", emoji: "map", targetAmount: $(1000), startDate: D(1), deadline: null, fundId, isActive: true })).id;
  });
  ...
  it("權限：一般內容雙方可改；獎金、懲罰、週期、基金、里程碑只有建立者能改；只有建立者能刪除；執行者停用當天未完成仍會被罰", async () => {
    ...
    await rejects(tasks.updateTask(c.ctxB, t.id, { ...general, milestones: [{ days: 3, bonusAmount: $(500), badgeEmoji: "medal", badgeName: "偷加", rewardText: "" }] }, D(10)), "TASK_CREATOR_ONLY");
