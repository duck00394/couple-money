...
describe("Phase 2：共同基金、共同目標（實際可用資金規則）", () => {
  ...
  it("投入必須選帳戶、不得超過帳戶可自由使用金額；投入不改變帳戶餘額、不影響誰欠誰", async () => {
    trip = (await funds.createFund(c.ctxA, { name: "日本旅遊基金", emoji: "✈️", targetAmount: $(30000), dueDate: "2027-03-01" })).id;
    rent = (await funds.createFund(c.ctxB, { name: "租屋基金", emoji: "🏠", targetAmount: $(50000), dueDate: null })).id;
  ...
  it("共同目標：目前金額 = 實際基金金額、剩餘金額、達成自動完成、改目標金額重新判斷", async () => {
    goalId = (await goals.createGoal(c.ctxA, { name: "日本旅行", description: "明年春天", emoji: "🌸", targetAmount: $(30000), startDate: "2026-09-01", deadline: "2027-03-31", fundId: trip, isActive: true })).id;
    ...
    await goals.updateGoal(c.ctxB, goalId, { name: "日本旅行", description: "", emoji: "🌸", targetAmount: $(40000), startDate: "2026-09-01", deadline: "2027-03-31", fundId: trip, isActive: true });
    v = (await goals.getGoal(c.ctxA, goalId))!;
    assert.equal(v.status, "ACTIVE");
  ...
  it("建立目標可同時建立同名基金；無基金目標可手動完成；停用", async () => {
    const g = await goals.createGoal(c.ctxB, { name: "新沙發", description: "", emoji: "🛋️", targetAmount: $(15000), startDate: "2026-09-17", deadline: null, fundId: "NEW", isActive: true });
    const linked = (await funds.listFunds(c.ctxA)).find((f) => f.name === "新沙發");
    assert.equal(g.fundId, linked!.id);
    const hike = await goals.createGoal(c.ctxA, { name: "一起爬玉山", description: "", emoji: "⛰️", targetAmount: $(1), startDate: "2026-09-17", deadline: null, fundId: null, isActive: true });
    await goals.setGoalAchieved(c.ctxA, hike.id, true);
    assert.equal((await goals.getGoal(c.ctxA, hike.id))!.status, "ACHIEVED");
    await goals.updateGoal(c.ctxA, hike.id, { name: "一起爬玉山", description: "", emoji: "⛰️", targetAmount: $(1), startDate: "2026-09-17", deadline: null, fundId: null, isActive: false });
