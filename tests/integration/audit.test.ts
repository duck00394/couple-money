...
describe("驗收 A：帳本隔離（Book 1 的資料，Book 2 完全碰不到）", () => {
  ...
  before(async () => {
    ...
    taskId = (await tasks.createTask(us.ctxA, {
      title: "英文", description: "", emoji: "📚", scope: "PERSONAL", assigneeId: us.aId, frequency: "DAILY", daysOfWeek: 127,
...
describe("驗收 B：財務不變式", () => {
  ...
  it("刪除任務會一起收回尚未入金的獎金與尚未抵扣的懲罰，基金才不會卡住", async () => {
    ...
    const task = await tasks.createTask(c.ctxA, {
      title: "運動", description: "", emoji: "🏃", scope: "PERSONAL", assigneeId: c.aId, frequency: "DAILY", daysOfWeek: 127,
  ...
  it("任務獎金在入金前不是現金：不影響帳戶、不影響實際基金金額", async () => {
    ...
    const task = await tasks.createTask(c.ctxA, {
      title: "早起", description: "", emoji: "☀️", scope: "PERSONAL", assigneeId: c.aId, frequency: "DAILY", daysOfWeek: 127,
  ...
  it("獎金入金不能從信用卡轉出", async () => {
    ...
    const task = await tasks.createTask(c.ctxA, {
      title: "閱讀", description: "", emoji: "📖", scope: "PERSONAL", assigneeId: c.aId, frequency: "DAILY", daysOfWeek: 127,
...
describe("驗收 C：並發（兩支手機同時操作）", () => {
  ...
  it("同時修改同一個基金：第二個會被樂觀鎖擋下（不會蓋掉對方）", async () => {
    ...
    const rs = await Promise.allSettled([
      funds.updateFund(c.ctxA, fund.id, { name: "小艾命名", emoji: "🐷", targetAmount: null, dueDate: null, isArchived: false }, version),
      funds.updateFund(c.ctxB, fund.id, { name: "阿本命名", emoji: "🐷", targetAmount: null, dueDate: null, isArchived: false }, version),
    ]);
  ...
  it("同時打卡同一個共同任務：只會有一筆打卡與一筆獎金", async () => {
    ...
    const task = await tasks.createTask(c.ctxA, {
      title: "一起運動", description: "", emoji: "🏃", scope: "SHARED", assigneeId: null, frequency: "DAILY", daysOfWeek: 127,
