...
describe("Phase 3-1：搜尋與篩選", () => {
  ...
  before(async () => {
    ...
    // 任務獎金入金（從阿本銀行轉入共同帳戶）
    const t = await tasks.createTask(c.ctxA, { title: "英文", description: "", emoji: "📚", scope: "PERSONAL", assigneeId: c.aId, frequency: "DAILY", daysOfWeek: 127, requiresApproval: false, requiresPhoto: false, rewardAmount: $(50), fundId: trip, penaltyAmount: 0, penaltyText: "", isActive: true, milestones: [] }, D(16));
