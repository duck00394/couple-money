...
describe("Phase 3-4 D：分類管理", () => {
  ...
  it("新增分類：名稱整理過、圖示有效、排在最後面", async () => {
    const created = await categories.createCategory(c.ctxA, { name: "  寵物  ", kind: "EXPENSE", icon: "🐶" });
  ...
  it("改名不會動到任何既有交易，舊紀錄跟著顯示新名稱", async () => {
    ...
    await categories.updateCategory(c.ctxA, foodId, { name: "吃飯", icon: "🍱" });
    ...
    await categories.updateCategory(c.ctxA, foodId, { name: "餐飲", icon: "🍜" }); // 改回來
  });
