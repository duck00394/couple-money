...
export async function v4Daily(a: Page, b: Page) {
  ...
  expect(home).toContain("今天已完成 2 次・+$60");
  expect(home, "首頁不該出現程式用語").not.toMatch(/PER_TIME|DAILY|WEEKLY/);
  await expect(a.getByRole("button", { name: /＋再完成一次 倒垃圾/ }).first()).toBeVisible();
  await a.getByRole("button", { name: /＋再完成一次 倒垃圾/ }).first().click();
  await expect(a.getByText("今天已完成 3 次・+$90").first()).toBeVisible();
  await shot(a, "52-home-dashboard");
  step("首頁 Dashboard：不用進詳細頁就能再完成一次（3 次・+$90）");

  // 7. 每次 × EACH：阿本自己做自己的
  await go(b, "/tasks");
  const benRow = b.getByTestId("task-row").filter({ hasText: "倒垃圾" }).first();
  await benRow.getByRole("button", { name: /打卡 倒垃圾/ }).click();
  await expect(b.getByText("今天已完成 1 次・+$30").first()).toBeVisible();
  const amyTasks = await pageText(a, "/tasks");
  expect(amyTasks, "小艾自己還是 3 次").toContain("今天已完成 3 次・+$90");
  step("每次 × 各自完成：小艾 3 次 $90、阿本 1 次 $30，互不影響");
  ...
  await expect(benWeek).toContainText("本週還可以完成一次");
