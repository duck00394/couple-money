...
async function main() {
  ...
  const trip = await funds.createFund(A, { name: "日本旅遊基金", emoji: "plane", targetAmount: $(30000), dueDate: addDays(today, 150) });
  const rent = await funds.createFund(B, { name: "租屋基金", emoji: "house", targetAmount: $(50000), dueDate: null });
  await funds.createFund(A, { name: "生日基金", emoji: "cake", targetAmount: $(10000), dueDate: addDays(today, 40) });
  ...
  await goals.createGoal(A, { name: "日本旅行", description: "明年春天去京都賞櫻", emoji: "map", targetAmount: $(30000), startDate: addDays(today, -14), deadline: addDays(today, 150), fundId: trip.id, isActive: true });
  await goals.createGoal(B, { name: "搬到大一點的家", description: "", emoji: "house", targetAmount: $(50000), startDate: addDays(today, -14), deadline: null, fundId: rent.id, isActive: true });
  ...
  const english = await tasks.createTask(A, { ...base, title: "英文 30 分鐘", emoji: "book", scope: "PERSONAL", assigneeId: a.id, rewardAmount: $(50), fundId: trip.id, penaltyAmount: $(20), penaltyText: "洗碗一次", milestones: ms });
  const run = await tasks.createTask(B, { ...base, title: "運動", emoji: "run", scope: "PERSONAL", assigneeId: b.id, rewardAmount: $(30), fundId: trip.id, requiresApproval: true, requiresPhoto: true, milestones: [] });
  const read = await tasks.createTask(A, { ...base, title: "睡前閱讀", emoji: "book", scope: "PERSONAL", assigneeId: a.id, frequency: "CUSTOM", daysOfWeek: 0b0111110, rewardAmount: $(20), fundId: trip.id, milestones: [] });
  const walk = await tasks.createTask(A, { ...base, title: "一起散步", emoji: "walk", scope: "SHARED", assigneeId: null, rewardAmount: $(20), fundId: rent.id, milestones: [] });
