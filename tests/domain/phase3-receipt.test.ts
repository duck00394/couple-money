...
import { assertImage, IMAGE_TYPES, MAX_AVATAR_BYTES, MAX_RECEIPTS, MAX_UPLOAD_BYTES, sizeLabel, sniffImage } from "../../src/server/domain/receipt";
...
describe("Phase 3-4 E：收據照片（純邏輯）", () => {
  ...
  it("頭貼用比較小的上限（2MB），而且訊息會說對數字", () => {
    assert.equal(MAX_AVATAR_BYTES, 2 * 1024 * 1024);
    assert.equal(sizeLabel(MAX_AVATAR_BYTES), "2MB");
    assert.equal(sizeLabel(MAX_UPLOAD_BYTES), "4MB");
    assert.equal(assertImage({ type: "image/png", size: MAX_AVATAR_BYTES }, PNG, MAX_AVATAR_BYTES), "png");
    fails(() => assertImage({ type: "image/png", size: MAX_AVATAR_BYTES + 1 }, PNG, MAX_AVATAR_BYTES), "UPLOAD_SIZE");
    assert.throws(
      () => assertImage({ type: "image/png", size: MAX_AVATAR_BYTES + 1 }, PNG, MAX_AVATAR_BYTES),
      /上限 2MB/,
      "錯誤訊息要說 2MB，不能寫死 4MB",
    );
    assert.equal(assertImage({ type: "image/png", size: MAX_AVATAR_BYTES + 1 }, PNG), "png", "收據仍然吃到 4MB");
  });
});
