    ...
    <>
      <form action="/transactions" className="flex gap-2" role="search">
        ...
        <button
          ...
          className={cx("press relative h-11 shrink-0 rounded-xl px-3 text-sm font-semibold shadow-sm", activeCount ? "bg-brand-500 text-white" : "bg-white text-stone-600")}
          aria-label="開啟篩選"
        >
        ...
        <div className="fade fixed inset-0 z-40 mx-auto flex max-w-md items-end bg-stone-900/35 backdrop-blur-[2px]" onClick={() => setOpen(false)} role="dialog" aria-modal="true" aria-label="篩選條件">
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => setOpen(false)} className="h-9 w-9 rounded-full text-xl text-stone-500" aria-label="關閉篩選">✕</button>
            </div>
