...
const STATUS: Record<string, { label: string; icon?: string; cls: string }> = {
  APPROVED: { label: "已完成", icon: "check", cls: "bg-emerald-50 text-emerald-700" },
...
export function TaskRow({ card, ctx, showSchedule }: { card: TaskCard; ctx: BookContext; showSchedule?: boolean }) {
  ...
  return (
    <div className={`flex items-center gap-3 px-4 py-3 ${task.isActive ? "" : "opacity-50"}`} data-testid="task-row">
      <Link href={`/tasks/${task.id}`} className="flex min-w-0 flex-1 items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="truncate text-xs text-stone-500">
            ...
            {stats.current > 0 && <span className="text-orange-600">・連續 {stats.current} 天</span>}
      ...
      {doable ? (
        task.requiresPhoto ? (
          <Link href={`/tasks/${task.id}`} className="press rounded-full bg-brand-200 ring-1 ring-brand-400/60 px-4 py-2 text-sm font-semibold text-stone-800 shadow-sm" aria-label={`打卡 ${task.title}`}>打卡</Link>
        ) : (
          <QuickCheckIn taskId={task.id} title={task.title} />
        )
      ) : status && card.scheduledToday ? (
        <Link href={`/tasks/${task.id}`} className={`flex items-center gap-1 rounded-full px-3 py-1.5 text-xs font-semibold ${status.cls}`} data-testid="task-status">
