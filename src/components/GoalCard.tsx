  ...
  return (
    <Link href={`/goals/${goal.id}`} className="block px-4 py-3 active:bg-stone-50" data-testid="goal-row">
      <div className="flex items-center gap-3">
        <span className={big ? "text-4xl" : "text-3xl"}>{goal.emoji}</span>
        ...
        <span className="tnum text-sm font-bold text-stone-800" data-testid="goal-percent">{goal.fund ? percentText(goal.current, goal.targetAmount) : achieved ? "100%" : "—"}</span>
      </div>
      {goal.fund ? (
...
export function FundRow({ fund }: { fund: FundView }) {
  return (
    <Link href={`/funds/${fund.id}`} className={`block px-4 py-3 active:bg-stone-50 ${fund.isArchived ? "opacity-50" : ""}`} data-testid="fund-row">
      <div className="flex items-center gap-3">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-stone-100 text-xl">{fund.emoji}</span>
        <div className="min-w-0 flex-1">
        ...
        <span className="tnum font-semibold text-stone-800">{formatMoney(fund.balance)}</span>
      </div>
