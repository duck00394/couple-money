    ...
    <Card className="space-y-3 text-sm" data-testid="tx-detail">
      ...
      {fund && (
        <p>
          ...
          <Link href={`/funds/${fund.fundId}`} className="text-brand-600 underline">{fund.fund.emoji} {fund.fund.name}</Link>（基金實際金額 −{formatMoney(tx.amount)}）
        </p>
      )}
      {tx.recurring && (
      ...
      <p className="border-t border-stone-100 pt-2 text-xs text-stone-400">
