...
function Row({ href, icon, iconBg = "bg-stone-100", title, sub, amount, amountClass = "", tags }: {
  href: string;
  icon: string;
  iconBg?: string;
  title: string;
  ...
  return (
    <Link href={href} className="flex items-center gap-3 px-4 py-3 active:bg-stone-50" data-testid="tx-row">
      <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl text-lg ${iconBg}`}>{icon}</span>
      ...
      <span className={`tnum shrink-0 text-[15px] font-semibold ${amountClass}`}>{amount}</span>
    </Link>
  );
}
...
export function TxRow({ tx, ctx }: { tx: TxListItem; ctx: BookContext }) {
  ...
  if (tx.type === "SETTLEMENT" && tx.settlement) {
    ...
    return <Row href={href} icon="🤝" iconBg="bg-emerald-50" title={`${name(ctx, s.fromUserId)} 還給 ${name(ctx, s.toUserId)}`} sub={`結算${tx.note ? ` · ${tx.note}` : ""}`} amount={formatMoney(tx.amount)} amountClass="text-emerald-600" />;
  }
  if (tx.type === "TRANSFER") {
  ...
  const fund = tx.fundEntry && !tx.fundEntry.deletedAt ? ` · ${tx.fundEntry.fund.emoji} ${tx.fundEntry.fund.name}` : "";
  const recurring = tx.recurring ? ` · 📅 固定支出` : "";
  return (
    <Row
      href={href}
      icon={isRefund ? "↩️" : tx.category?.icon ?? (isIncome ? "💵" : "🏷️")}
      iconBg={isIncome || isRefund ? "bg-emerald-50" : "bg-stone-100"}
      title={title}
