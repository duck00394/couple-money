...
export function MoneyConcepts({ open = false }: { open?: boolean }) {
  const rows = [
    ["💵", "個人帳戶", "錢實際放在誰身上（現金、銀行、信用卡）。用個人帳戶付錢，會照分帳產生「誰欠誰」。"],
    ["💞", "共同帳戶", "兩人一起的錢實際放在哪裡。從共同帳戶付錢，不會產生個人欠款。"],
    ["🐷", "共同基金", "把帳戶裡的錢「指定用途」，例如旅遊、租屋。投入不會移動錢、不算欠款，但不能超過帳戶「可自由使用」的金額。"],
    ["🏆", "尚未入金獎金", "完成任務獲得的獎金（扣掉懲罰），只是兩人的承諾，還不是現金。到基金頁「入金」後才變成實際基金金額。"],
    ["🎯", "共同目標", "想達成的結果。目前金額 = 連結基金的實際金額；尚未入金獎金另外顯示。"],
  ];
  return (
