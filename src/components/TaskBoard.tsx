          ...
          {cards.map((c, i) => {
            ...
            return (
              ...
              >
                ...
                <p className="truncate text-[13px] font-extrabold leading-tight text-stone-800">{c.task.title}</p>
                <p className="mt-0.5 truncate text-[9.5px] text-stone-500">
                  {who(c)}
                  {c.stats.current > 0 && `・連續 ${c.stats.current} 天`}
                </p>
                {c.task.rewardAmount > 0 && (
                  <p className="mt-0.5 text-[10px] font-bold text-brand-600">+${Math.round(c.task.rewardAmount / 100)}</p>
                )}
