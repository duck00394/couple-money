  ...
  return (
    <div className="px-4 py-3" data-testid="category-row">
      <div className="flex items-center gap-3">
        <ArtTile name={category.icon} size={36} />
        ...
        {canWrite && <div className="flex shrink-0 items-center gap-2 text-xs">
          <button className="-my-1 px-1.5 py-1.5 text-stone-400 underline" onClick={() => setEditing((v) => !v)} aria-label={`編輯 ${category.name}`} disabled={busy}>編輯</button>
          <form action={archive} className="inline">
            <input type="hidden" name="id" value={category.id} />
            <input type="hidden" name="archived" value={String(!category.isArchived)} />
            <button className="-my-1 px-1.5 py-1.5 text-stone-400 underline" disabled={busy} aria-label={`${category.isArchived ? "啟用" : "停用"} ${category.name}`}>
          ...
          {category.deletable && (
            ...
            >
              <input type="hidden" name="id" value={category.id} />
              <button className="-my-1 px-1.5 py-1.5 text-red-500 underline" disabled={busy} aria-label={`刪除 ${category.name}`}>刪除</button>
            </form>
          )}
        </div>}
      </div>
      ...
      {editing && (
        <ActionForm action={action} className="mt-2 space-y-2 rounded-xl bg-stone-50 p-3" data-testid="category-edit">
          ...
          <Field label="圖示">
            <IconPicker name="icon" options={CATEGORY_ICONS} defaultValue={category.icon} />
          </Field>
...
export function NewCategoryForm() {
  ...
  return (
    <ActionForm action={action} className="space-y-3 rounded-2xl bg-white p-4 shadow-sm" data-testid="new-category">
      ...
      <Field label="圖示">
        <IconPicker name="icon" options={CATEGORY_ICONS} />
      </Field>
      <ErrorText>{state?.error}</ErrorText>
