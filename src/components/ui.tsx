...
export function Button({
  ...
  return (
    <button
      className={cx(
        "press flex h-12 items-center justify-center gap-2 rounded-xl px-4 text-base font-extrabold transition-all disabled:cursor-not-allowed disabled:active:scale-100 disabled:active:translate-y-0",
        btn[variant],
        className,
      )}
      {...props}
...
export function LinkButton({
  ...
  return (
    <Link
      className={cx("press flex h-12 items-center justify-center gap-2 rounded-xl px-4 text-base font-extrabold transition-all", btn[variant], className)}
      {...props}
    />
  );
}
...
export function Avatar({ name, color, size = 36, src }: { name: string; color: string; size?: number; src?: string | null }) {
  ...
  return (
    <span
      className="inline-flex shrink-0 items-center justify-center rounded-full border-2 border-stone-800 font-extrabold text-stone-800"
...
export function PageHeader({ title, back, right }: { title: string; back?: string; right?: ReactNode }) {
  return (
    <header className="sticky top-0 z-10 flex h-14 items-center gap-2 bg-canvas/90 px-4 backdrop-blur-md">
      ...
      <h1 className="flex-1 truncate text-lg font-extrabold tracking-tight text-stone-800">{title}</h1>
      {right}
    </header>
  );
}
...
export function SectionTitle({ children, right }: { children: ReactNode; right?: ReactNode }) {
  return (
    <div className="mb-2.5 mt-8 flex items-center justify-between gap-3 px-1">
      <h2 className="text-[13px] font-extrabold tracking-wide text-stone-700">{children}</h2>
      {right}
    </div>
  );
}
...
export function Collapsible({ title, children, className, open }: { title: string; children: ReactNode; className?: string; open?: boolean }) {
  return (
    <details className={cx("group mt-6", className)} open={open}>
      <summary className="paper mb-2 flex cursor-pointer list-none items-center justify-between px-4 py-3.5 text-sm font-bold text-stone-700">
        {title}
