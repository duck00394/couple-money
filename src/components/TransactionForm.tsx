  ...
  return (
    <div className="pb-submit-bar px-4">
      <form action={action} className="space-y-5">
        ...
        <div className="rounded-3xl bg-white px-5 py-4 shadow-sm ring-1 ring-brand-100">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <input
              ...
              placeholder="0"
              className="tnum w-full bg-transparent text-5xl font-bold leading-tight text-stone-800 outline-none placeholder:text-stone-300"
            />
          </div>
              ...
              <optgroup key={g.label} label={g.label}>
                {g.items.map((a) => (
                  <option key={a.id} value={a.id}>
                    {a.icon} {g.label === "共同" ? "" : `${g.label}・`}{a.name}
                  </option>
                ))}
              </optgroup>
            ))}
