...
export default async function GoalsPage({ searchParams }: PageProps<"/goals">) {
  ...
  return (
    <>
      <PageHeader title="目標" right={<Link href="/goals/new" className="rounded-full bg-brand-500 px-3 py-1.5 text-sm font-semibold text-white">＋ 目標</Link>} />
      <div className="px-4">
        {toMe.length > 0 && (
          <>
        ...
        <SectionTitle>共同目標</SectionTitle>
        ...
        {done.length > 0 && (
          <>
            ...
            <Card className="divide-y divide-stone-100 p-0">{done.map((g) => <GoalRow key={g.id} goal={g} />)}</Card>
          </>
        )}
