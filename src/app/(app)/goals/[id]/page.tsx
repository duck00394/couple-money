  ...
  return (
    <>
      ...
      <div className="px-4">
        ...
        {goal.fund ? (
          <Link href={`/funds/${goal.fund.id}`} className="mt-3 flex items-center gap-3 rounded-2xl bg-white p-4 shadow-sm active:bg-stone-50">
            <span className="text-2xl">{goal.fund.emoji}</span>
          ...
          <>
            ...
            {goal.fund ? (
              <Card className="text-sm text-stone-500">
                這個目標連結了基金「{goal.fund.emoji} {goal.fund.name}」，實際基金金額達到 {formatMoney(goal.targetAmount)} 就會自動完成，不需要手動標記。
              </Card>
            ) : (
              <GoalStatusButtons id={goal.id} achieved={achieved} />
            )}
            <Collapsible title="✏️ 編輯或刪除目標">
            <Card>
              <GoalForm
                funds={funds.map((f) => ({ id: f.id, name: `${f.emoji} ${f.name}${f.isArchived ? "（封存）" : ""}` }))}
                values={{
                  id: goal.id,
                  name: goal.name,
