...
export default async function TransactionsPage({ searchParams }: PageProps<"/transactions">) {
  ...
  return (
    <>
      <PageHeader
        title="記帳"
        right={<Link href="/transactions/new?from=/transactions" className="press rounded-full bg-brand-500 px-3.5 py-2 text-sm font-semibold text-white shadow-sm active:bg-brand-600">＋ 記一筆</Link>}
      />
      <div className="px-4">
        ...
        {filtering ? (
          ...
          <div className="mt-3 grid grid-cols-4 gap-2 text-center">
            {[
              { href: "/transactions/transfer", icon: "🔁", label: "轉帳" },
              { href: "/transactions/refund", icon: "↩️", label: "退款" },
              { href: "/recurring", icon: "📅", label: "固定支出", badge: duePending.length },
              { href: "/settle", icon: "🤝", label: "結算" },
              ...
              <Link
                key={s.href}
                href={s.href}
                className="press relative flex flex-col items-center gap-1 rounded-2xl bg-white px-1 py-2.5 shadow-sm active:bg-stone-50"
              >
                <span className="text-lg leading-none">{s.icon}{" "}</span>
                <span className="text-[11px] leading-none text-stone-600">{s.label}</span>
                {!!s.badge && (
                  <span className="absolute right-1 top-1 rounded-full bg-brand-500 px-1.5 text-[10px] font-semibold leading-4 text-white">{s.badge}</span>
                )}
              </Link>
            ))}
            <a
              ...
              download
              className="press flex flex-col items-center gap-1 rounded-2xl bg-white px-1 py-2.5 shadow-sm active:bg-stone-50"
        ...
        {filtering && (
          <Card className="mt-3 grid grid-cols-3 gap-2 px-3 py-3.5 text-center" data-testid="search-totals">
            <div><p className="text-[11px] text-stone-500">共</p><p className="tnum font-bold">{totals.count} 筆</p></div>
            <div><p className="text-[11px] text-stone-500">實際淨支出</p><p className="tnum font-bold" data-testid="search-net-expense">{formatMoney(totals.netExpense)}</p></div>
            <div><p className="text-[11px] text-stone-500">收入</p><p className="tnum font-bold text-emerald-600">{formatMoney(totals.income)}</p></div>
        ...
        {filtering && (
          <a
            ...
            download
            className="press mt-2 block rounded-2xl bg-white py-3 text-center text-sm font-semibold text-brand-600 shadow-sm active:bg-stone-50"
        ...
        <div className="mt-4">
          {items.length === 0 && (
            <Card className="p-0"><Empty icon={filtering ? "🔍" : "🧾"}>{filtering ? "找不到符合條件的紀錄" : "還沒有任何紀錄"}</Empty></Card>
          )}
