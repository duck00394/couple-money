...
import { Card, Empty, PageHeader, ProgressBar, SectionTitle } from "@/components/ui";
...
export default async function TasksPage() {
  ...
  const [board, reviews, penalties, badges, rewards] = await Promise.all([taskBoard(ctx), pendingReviews(ctx), recentPenalties(ctx, 5), listBadges(ctx), todayRewards(ctx)]);
  ...
  return (
    <>
      <PageHeader title="任務" right={<Link href="/tasks/new" className="rounded-full bg-brand-500 px-3 py-1.5 text-sm font-semibold text-white">＋ 新任務</Link>} />
      <div className="px-4">
        <div className="grid grid-cols-2 gap-2">
          <Card className="px-3 py-3">
            ...
            <p className="mt-1 text-[11px] text-stone-400">{board.weekMine.done}/{board.weekMine.scheduled} 次</p>
          </Card>
          <Card className="px-3 py-3">
                ...
                <div key={r.id} className="flex items-center gap-3 px-4 py-3">
                  <Link href={`/tasks/${r.taskId}`} className="flex min-w-0 flex-1 items-center gap-3">
                    {r.photoId ? (
                      ...
                      <span className="text-xl">{r.task.emoji}</span>
                    )}
        ...
        <Card className="divide-y divide-stone-100 p-0">
          {board.today.length === 0 ? <Empty>今天沒有排定的任務</Empty> : board.today.map((c) => <TaskRow key={c.task.id} card={c} ctx={ctx} />)}
        </Card>
            ...
            <Card className="grid grid-cols-3 gap-2 text-center">
              {streaks.map((c) => (
                <Link key={c.task.id} href={`/tasks/${c.task.id}`} className="rounded-xl bg-orange-50 px-2 py-3">
                  <p className="text-2xl">{c.task.emoji}</p>
        ...
        {groups.map((g) => {
          ...
          return (
            <section key={g.key}>
              ...
              <Card className="divide-y divide-stone-100 p-0">
                {list.length === 0 ? <Empty>沒有任務</Empty> : list.map((c) => <TaskRow key={c.task.id} card={c} ctx={ctx} showSchedule />)}
              </Card>
            </section>
          );
        })}
            ...
            <Card className="flex flex-wrap gap-2">
              {badges.slice(0, 12).map((b) => (
                <span key={b.id} className="rounded-full bg-amber-50 px-3 py-1 text-xs text-amber-800" title={b.earnedAt.toISOString()}>
                  {b.emoji} {b.title}・{b.subjectKey === "COUPLE" ? "共同" : name(b.subjectKey)}
                </span>
              ))}
            </Card>
          </>
