...
const MENU: Array<{ title: string; items: Array<{ href: string; icon: string; label: string; sub?: string }> }> = [
  {
    title: "錢",
    items: [
      { href: "/transactions", icon: "🧾", label: "記帳明細", sub: "搜尋、篩選、批次整理" },
      { href: "/stats", icon: "📊", label: "統計與報表", sub: "誰掏錢、誰負擔、分類佔比" },
      { href: "/budgets", icon: "🎯", label: "每月預算", sub: "只是提醒，不會擋記帳" },
      { href: "/categories", icon: "🏷️", label: "分類管理", sub: "新增、改名、停用" },
      { href: "/settle", icon: "🤝", label: "結算", sub: "把欠款一次結清" },
      { href: "/accounts", icon: "💳", label: "帳戶與餘額", sub: "錢放在哪裡" },
      { href: "/goals", icon: "🐷", label: "共同基金與目標", sub: "錢要做什麼" },
    ],
  },
  {
    title: "一起",
    items: [
      { href: "/tasks", icon: "✅", label: "每日任務與徽章", sub: "打卡、連續天數、獎金" },
      { href: "/activity", icon: "🔔", label: "最近動態", sub: "另一半做了什麼" },
    ],
  },
];
    ...
    <>
      ...
      <div className="px-4">
        <Card className="flex items-center gap-3 p-4">
          <Avatar name={ctx.me.nickname} color={ctx.me.avatarColor} size={44} />
          <div className="min-w-0 flex-1">
            <p className="font-semibold text-stone-800">{ctx.me.nickname}</p>
        ...
        <Card>
          {ctx.partner ? (
            <div className="flex items-center gap-3">
              <Avatar name={ctx.partner.nickname} color={ctx.partner.avatarColor} />
        ...
        {MENU.map((section) => (
          <div key={section.title}>
            ...
            <Card className="divide-y divide-stone-100 p-0">
              {section.items.map((i) => (
                <ListLink key={i.href} href={i.href} icon={i.icon} sub={i.sub}>{i.label}</ListLink>
              ))}
            </Card>
          </div>
        ))}
