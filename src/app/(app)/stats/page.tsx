      ...
      <div className="px-4">
        ...
        {!hasAnything ? (
          <Card className="mt-3 p-0">
            <Empty icon="🍃" action={<Link href="/transactions/new" className="text-sm font-semibold text-brand-600">去記一筆 →</Link>}>
              {label(month)}還沒有任何紀錄
            </Empty>
          </Card>
        ) : (
          <>
            <Card className="mt-3 p-0">
              <Link href={drill()} className="block px-5 py-4 active:bg-stone-50">
                ...
                <p className="tnum mt-0.5 text-4xl font-bold leading-tight text-stone-800" data-testid="stats-net-expense">{formatMoney(net)}</p>
              ...
              <Link href={drill({ kind: "INCOME" })} className="flex items-baseline justify-between border-t border-stone-100 px-5 py-3.5 active:bg-stone-50">
                <span className="text-sm text-stone-500">收入<span className="ml-2 text-[11px] text-stone-400">不含轉帳與結算</span></span>
                <span className="tnum text-xl font-bold text-emerald-600" data-testid="stats-income">{formatMoney(stats.totals.income)}</span>
              </Link>
            </Card>
            ...
            <Card className="divide-y divide-stone-100 p-0">
              {funds.map((f) => (
                <div key={f.id} className="flex items-baseline justify-between gap-2 px-4 py-3 text-sm" data-testid="stats-fund">
                  <span className="min-w-0 flex-1 truncate">{f.emoji} {f.name}</span>
