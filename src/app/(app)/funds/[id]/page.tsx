...
const ICON: Record<string, string> = { DEPOSIT: "piggy-bank", WITHDRAW: "undo", EXPENSE: "transaction", REWARD_DEPOSIT: "trophy" };
  ...
  return (
    <>
      ...
      <div className="px-4">
          ...
          {entries.map((e) => {
            ...
            return (
              <div key={e.id} className="flex items-center gap-3 px-4 py-3" data-testid="fund-entry">
                <ArtTile name={ICON[e.type] ?? "tag"} size={36} />
        ...
        {ctx.canWrite && (
          <Collapsible title="基金設定（名稱、目標金額、封存、刪除）">
            <Card>
              <FundForm
                values={{
                  id: fund.id,
