...
export const DEFAULT_MILESTONES = [
  { days: 3, badgeEmoji: "🌱", badgeName: "三天好的開始" },
  { days: 7, badgeEmoji: "🔥", badgeName: "一週連續" },
  { days: 14, badgeEmoji: "💪", badgeName: "兩週不間斷" },
  { days: 30, badgeEmoji: "🏆", badgeName: "30 天達人" },
];
...
export const subjectKeyOf = (task: Pick<Task, "scope" | "assigneeId">) => (task.scope === "SHARED" ? COUPLE : task.assigneeId!);
...
async function validateTask(client: Tx | typeof prisma, ctx: BookContext, input: TaskInput) {
  ...
  assert(input.scope === "PERSONAL" || input.scope === "SHARED", "TASK_SCOPE", "任務類型不正確");
  ...
  const needsFund = input.rewardAmount > 0 || input.penaltyAmount > 0 || input.milestones.some((m) => m.bonusAmount > 0);
  assert(!needsFund || input.fundId, "TASK_FUND", "有獎金或扣款的任務，請選擇對應的基金");
  if (input.fundId) {
  ...
  return {
    ...
    scope: input.scope,
    assigneeId: input.scope === "PERSONAL" ? input.assigneeId : null,
...
export async function updateTask(ctx: BookContext, taskId: string, input: TaskInput, today = toDateKey(new Date()), expectedUpdatedAt?: string | null) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (before.isActive && !data.isActive && hasPenalty(before) && before.penaltyStartDate) {
      const subjectKey = subjectKeyOf(before);
      const isExecutor = before.scope === "SHARED" || before.assigneeId === ctx.me.userId;
      ...
      if (isExecutor && due) {
        ...
        if (!done && !already) await createPenalty(tx, ctx, before, today, "停用任務時今天尚未完成");
      }
    }
    ...
    for (const m of input.milestones) {
      const data2 = { bonusAmount: m.bonusAmount, badgeEmoji: m.badgeEmoji.trim() || "🏅", badgeName: m.badgeName.trim(), rewardText: m.rewardText.trim() || null };
...
export async function deleteTask(ctx: BookContext, taskId: string) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const pendingRewards = await tx.taskReward.findMany({ where: { taskId, deletedAt: null, depositEntryId: null }, select: { id: true } });
    if (pendingRewards.length) {
      await tx.taskReward.updateMany({ where: { id: { in: pendingRewards.map((r) => r.id) } }, data: { deletedAt: now } });
    }
    await tx.taskPenalty.updateMany({ where: { taskId, waivedAt: null, depositEntryId: null }, data: { waivedAt: now, waivedById: ctx.me.userId } });
...
async function giveReward(
  ...
  if (opts.amount <= 0 || !opts.task.fundId) return null;
...
export async function checkIn(ctx: BookContext, taskId: string, opts: { note?: string; photoId?: string | null; today?: string } = {}) {
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      const subjectKey = subjectKeyOf(task);
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const task of tasks) {
      ...
      for (const day of eachDay(from, yesterday)) {
        ...
        await createPenalty(tx, ctx, task, day, "未完成");
        created++;
      }
    }
  });
...
async function createPenalty(tx: Tx, ctx: BookContext, task: Task, day: string, reason: string) {
  const offender = task.scope === "PERSONAL" ? task.assigneeId : null;
  await tx.taskPenalty.create({
    data: {
      bookId: ctx.book.id,
      taskId: task.id,
      subjectKey: subjectKeyOf(task),
      userId: offender,
      date: keyToDbDate(day),
      amount: task.fundId ? task.penaltyAmount : 0,
...
export async function taskBoard(ctx: BookContext, today = toDateKey(new Date())) {
  ...
  const cards: TaskCard[] = tasks.map((task) => {
    const subjectKey = subjectKeyOf(task);
    const mine = checkIns.filter((c) => c.taskId === task.id && c.subjectKey === subjectKey);
    const todayCi = mine.find((c) => dbDateToKey(c.date) === today && c.status !== "CANCELLED") ?? null;
    const approved = new Set(mine.filter((c) => c.status === "APPROVED").map((c) => dbDateToKey(c.date)));
    const group: TaskGroup = task.scope === "SHARED" ? "SHARED" : task.assigneeId === ctx.me.userId ? "MINE" : "PARTNER";
...
export async function recentPenalties(ctx: BookContext, take = 10) {
  return prisma.taskPenalty.findMany({
    ...
    where: { bookId: ctx.book.id, waivedAt: null, depositEntryId: null, task: { deletedAt: null } },
    include: { task: true },
    orderBy: { date: "desc" },
    take,
  });
