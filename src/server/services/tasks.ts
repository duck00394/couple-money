...
import { isWithdrawn, NOT_WITHDRAWN } from "../domain/withdrawal";
...
export async function deleteTask(ctx: BookContext, taskId: string) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const pendingRewards = await tx.taskReward.findMany({ where: { taskId, deletedAt: null, depositEntryId: null, ...NOT_WITHDRAWN }, select: { id: true } });
    if (pendingRewards.length) {
      await tx.taskReward.updateMany({ where: { id: { in: pendingRewards.map((r) => r.id) } }, data: { deletedAt: now } });
    }
    await tx.taskPenalty.updateMany({ where: { taskId, waivedAt: null, depositEntryId: null, ...NOT_WITHDRAWN }, data: { waivedAt: now, waivedById: ctx.me.userId } });
...
async function revokeRewards(tx: Tx, ctx: BookContext, checkInId: string) {
  const now = new Date();
  const rewards = await tx.taskReward.findMany({ where: { checkInId, deletedAt: null }, include: { withdrawal: { select: { deletedAt: true } } } });
  assert(!rewards.some(isWithdrawn), "REWARD_WITHDRAWN", "這次打卡的獎勵已經提列成收入了，請先作廢那筆收入紀錄");
...
export async function waivePenalty(ctx: BookContext, penaltyId: string) {
  ...
  await prisma.$transaction(async (tx) => {
    await lockBook(tx, ctx.book.id);
    const p = await tx.taskPenalty.findFirst({ where: { id: penaltyId, bookId: ctx.book.id }, include: { withdrawal: { select: { deletedAt: true } } } });
    ...
    assert(!isWithdrawn(p), "PENALTY_SETTLED", "這筆懲罰已經在提列時扣掉了，不能再免除");
...
export async function recentPenalties(ctx: BookContext, take = 10) {
  return prisma.taskPenalty.findMany({
    ...
    where: { bookId: ctx.book.id, waivedAt: null, depositEntryId: null, ...NOT_WITHDRAWN, task: { deletedAt: null } },
    include: { task: true },
    orderBy: { date: "desc" },
    take,
  });
