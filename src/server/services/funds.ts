...
import { NOT_WITHDRAWN } from "../domain/withdrawal";
...
export async function pendingByFund(client: Client, bookId: string, fundIds?: string[]) {
  // 獎勵離開餘額有兩個出口：獎金入金（depositEntryId）與提列成收入（withdrawalId）。
  // 兩個都要排除，否則已經提列過的獎勵還會顯示成「尚未入金」，然後被入金第二次。
  const where = { bookId, depositEntryId: null, ...NOT_WITHDRAWN, ...(fundIds ? { fundId: { in: fundIds } } : { fundId: { not: null } }) };
...
export async function depositRewards(
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      assert(fund, "FUND_NOT_FOUND", "找不到基金");
      const [rewards, penalties] = await Promise.all([
        // 已經提列成收入的不能再入金一次（同一筆獎勵只能有一個出口）
        tx.taskReward.findMany({ where: { fundId: fund.id, deletedAt: null, depositEntryId: null, ...NOT_WITHDRAWN } }),
        tx.taskPenalty.findMany({ where: { fundId: fund.id, waivedAt: null, depositEntryId: null, amount: { gt: 0 }, ...NOT_WITHDRAWN } }),
      ]);
